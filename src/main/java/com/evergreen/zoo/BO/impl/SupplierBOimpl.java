...
import com.evergreen.zoo.dao.SupplierDAOimpl;
...
public class SupplierBOimpl implements Initializable {
    ...
    void loadSuppliers() {
        ArrayList<SupplierDto> supplierDtos = supplierDAOimpl.getSuppliers();
    ...
    void addSupplier(ActionEvent event) {
        ...
        Boolean isAdded = supplierDAOimpl.addSupplier(supplierDto);
        if (isAdded) {
    ...
    void deleteSupplier(ActionEvent event) throws SQLException {
        ...
        ArrayList<FoodDto> items = supplierDAOimpl.getSupplierItems(supplierDto.getSupplierID());
        if(items.size() > 0){
            ...
            if(alert.getResult().getText().equals("Cancel")){
                ...
                Boolean isDeleted = supplierDAOimpl.isDeleteSupplier(supplierDto , items);
                if (isDeleted) {
    ...
    void updateSupplier(ActionEvent event) {
        ...
        String description = supDescriptionTXT.getText();
        SupplierDto supplierDto = new SupplierDto(name, phone, email, address, description);
        Boolean isUpdated = supplierDAOimpl.isUpdateSupplier(supplierDto);
        if (isUpdated) {
